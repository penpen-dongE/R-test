...
#id 변수 제거
df_breast_cancer <- df_breast_cancer %>% dplyr::select(-id)
View(df_breast_cancer)
...
clf <- xgboost(data = data.matrix(trainset %>% dplyr::select(-diagnosis)),
...
pred = predict(clf, data.matrix(testset %>% dplyr::select(-diagnosis)))
View(pred)
